...
(function () {
  ...
  // Autowire: Button mit data-scan-target="<input-id>" scannt und schickt dessen Formular ab.
  document.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-scan-target]');
    if (!btn) return;
    const input = document.getElementById(btn.dataset.scanTarget);
    window.startBarcodeScan((code) => { input.value = code; input.form.requestSubmit(); });
  });
})();
